...
CreateThread(function()
	while true do
		...
		local ped = cache.ped
		if IsPedInAnyVehicle(ped, false) and seat == -1 then
			if not GetIsVehicleEngineRunning(vehicle) then
				local isOpen, _ = lib.isTextUIOpen()
				if not isOpen then
					lib.showTextUI('[RSHIFT] to turn engine on')
				end
			else
				local isOpen, text = lib.isTextUIOpen()
				if isOpen and text == '[RSHIFT] to turn engine on' then
					lib.hideTextUI()
				end
			end
		else
...
togglelocksBind = lib.addKeybind({
	...
	defaultKey = config.keybinds.toggleLocks,
	onPressed = function()
